...
if [[ -z "$PULP_API_URL" ]]; then
  echo "You must provide PULP_API_URL environment variable" 1>&2
  exit 1
fi
...
if [[ -z "${NODE_EXTRA_CA_CERTS}" ]]; then
  ...
  # Copy the Kube API and service CA bundle to NODE_EXTRA_CA_CERTS if they exist
